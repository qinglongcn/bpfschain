...
// TestOpcodeDisabled 手动测试opcodeDisabled函数，因为所有禁用的操作码在正常执行时都会导致脚本执行失败，所以正常情况下不会调用该函数。
...
// TestOpcodeDisasm 在单行和完整模式下测试所有操作码的打印函数，以确保它提供预期的反汇编。
func TestOpcodeDisasm(t *testing.T) {
	t.Parallel()

	// 首先，测试oneline反汇编。

	// 数据推送操作码的预期字符串在下面的测试循环中被替换，因为它们涉及重复字节。
	// 另外，下面也替换了 OP_NOP# 和 OP_UNKNOWN#，因为这比在此处手动列出它们更容易。
	oneBytes := []byte{0x01}
	oneStr := "01"
	...
	for opcodeVal, expectedStr := range expectedStrings {
		var data []byte
		switch {
		// OP_DATA_1 到 OP_DATA_65 显示推送的数据。
		...
		case opcodeVal >= 0xb0 && opcodeVal <= 0xb9:
			switch opcodeVal {
			case 0xb1:
				// OP_NOP2 是 OP_CHECKLOCKTIMEVERIFY 的别名
				expectedStr = "OP_CHECKLOCKTIMEVERIFY"
			case 0xb2:
				// OP_NOP3 是 OP_CHECKSEQUENCEVERIFY 的别名
	...
	for opcodeVal, expectedStr := range expectedStrings {
		var data []byte
		switch {
		// OP_DATA_1 到 OP_DATA_65 显示操作码，后跟推送的数据。
		case opcodeVal >= 0x01 && opcodeVal < 0x4c:
			data = bytes.Repeat(oneBytes, opcodeVal)
		...
		case opcodeVal >= 0xba && opcodeVal <= 0xf9 || opcodeVal == 0xfc:
			switch opcodeVal {
			// OP_UNKNOWN186 又名 0xba 现在是 OP_CHECKSIGADD。
			case 0xba:
