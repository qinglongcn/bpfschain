...
// RawTxInSignature 返回给定交易的输入 idx 的序列化 ECDSA 签名，并附加 hashType。
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	case NullDataTy:
		return nil, class, nil, 0,
			fmt.Errorf("can't sign NULLDATA transactions")
	default:
		return nil, class, nil, 0,
			fmt.Errorf("can't sign unknown transactions")
	}
}
...
// SignTxOutput 对给定 tx 的输出 idx 进行签名，以解析 pkScript 中给定的脚本，签名类型为 hashType。
// 将通过使用给定地址的字符串调用 getKey() 来查找所需的任何密钥。
// 任何 pay-to-script-hash 签名都将通过调用 getScript 进行类似的查找。
// 如果提供了 previousScript，则 previousScript 中的结果将以类型相关的方式与新生成的结果合并。
// 签名脚本。
//
