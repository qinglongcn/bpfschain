...
// mustParseShortForm 解析传递的短格式脚本并返回结果字节。
// 如果发生错误，它会发生恐慌。
// 这仅在测试中用作帮助程序，因为它失败的唯一方法是测试源代码中存在错误。
...
// newAddressPubKey 从提供的序列化公钥返回一个新的 btcutil.AddressPubKey。
// 如果发生错误，它会发生恐慌。
// 这仅在测试中用作帮助程序，因为它失败的唯一方法是测试源代码中存在错误。
...
// newAddressPubKeyHash 从提供的哈希返回一个新的 btcutil.AddressPubKeyHash。
// 如果发生错误，它会发生恐慌。
// 这仅在测试中用作帮助程序，因为它失败的唯一方法是测试源代码中存在错误。
...
// newAddressScriptHash 从提供的哈希返回一个新的 btcutil.AddressScriptHash。
// 如果发生错误，它会发生恐慌。
// 这仅在测试中用作帮助程序，因为它失败的唯一方法是测试源代码中存在错误。
...
// newAddressTaproot 从提供的哈希返回一个新的 btcutil.AddressTaproot。
// 如果发生错误，它会发生恐慌。
// 这仅在测试中用作帮助程序，因为它失败的唯一方法是测试源代码中存在错误。
...
// TestExtractPkScriptAddrs 确保从 PkScripts 中提取所需签名的类型、地址和数量按预期进行。
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		// 由于无效的公钥、解析失败以及不是标准形式等原因，以下是非标准脚本。
		...
		// 请注意，从技术上讲，公钥是堆栈上的第二项，但由于地址提取有意仅适用于标准 PkScript，因此不应返回任何地址。
		{
		...
		// 来自真实 tx 691dd277dc0e90a462a3d652a1171686de49cf19067cd33c7df0392833fb986a，vout 0 无效公钥
		{
...
// TestCalcScriptInfo 确保 CalcScriptInfo 为各种有效和无效脚本对提供预期结果。
func TestCalcScriptInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// 来自区块链的 567a53d1ce19ce3d07711885168484439965501536d0d0294c5d46d46c10e53b。
		...
		},
		{
			...
			// OP_CHECKMULTISIG 错误的末尾有额外的 0 arg。
			sigScript: "1 1 1 0",
			pkScript: "3 " +
...
// bogusAddress 实现 btcutil.Address 接口，以便测试可以确保正确处理不支持的地址类型。
type bogusAddress struct{}

// EncodeAddress 只是返回一个空字符串。 它的存在是为了满足 btcutil.Address 接口。
func (b *bogusAddress) EncodeAddress() string {
	return ""
}

// ScriptAddress 简单地返回一个空字节片。 它的存在是为了满足 btcutil.Address 接口。
func (b *bogusAddress) ScriptAddress() []byte {
	return nil
}

// IsForNet 公然谎言是为了满足 btcutil.Address 接口。
func (b *bogusAddress) IsForNet(chainParams *chaincfg.Params) bool {
	return true // 为什么不？
}

// String 只是返回一个空字符串。 它的存在是为了满足 btcutil.Address 接口。
func (b *bogusAddress) String() string {
	return ""
}

// TestPayToAddrScript 确保 PayToAddrScript 函数为各种类型的地址生成正确的脚本。
func TestPayToAddrScript(t *testing.T) {
	...
	// 主网p2pk  13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
...
// TestMultiSigScript 确保 MultiSigScript 函数返回预期的脚本和错误。
...
// TestCalcMultiSigStats 确保 CalcMutliSigStats 函数返回预期的错误。
...
// scriptClassTests 包含多个测试脚本，用于确保各种类别确定按预期工作。
// 它被定义为全局测试与函数范围内的测试，因为这涵盖了标准测试和共识测试（付费脚本哈希是共识的一部分）。
var scriptClassTests = []struct {
	...
	// 部分 tx 6d36bc17e947ce00bb6f12f8e7a56a1585c5a36188ffa2b05e10b4743273a74b 代码分隔符部分已被省略。
	// （比特币核心对多重签名类型的检查也没有codesep）。
	{
		name: "multisig",
	...
	{
		// 几乎为空数据，但在数据后添加额外的操作码以使其非标准。
		name:   "almost nulldata",
		script: "RETURN 4 TRUE",
		class:  NonStandardTy,
	},

	// 接下来的几个几乎是多重签名（它是更复杂的脚本类型），但进行了各种更改以使其失败。
	{
		// 多重签名但 nsig 无效。
		name: "strange 1",
...
// TestScriptClass 确保 scriptClassTests 中的所有脚本都具有预期的类。
...
// TestStringifyClass 确保脚本类字符串返回每个脚本类的预期字符串。
...
// TestNullDataScript 测试 NullDataScript 是否返回有效的脚本。
...
// TestNewScriptClass 测试 NewScriptClass 是否返回有效的 ScriptClass。
