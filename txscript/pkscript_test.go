...
// TestParsePkScript 确保支持的脚本类型可以被正确解析并重新派生为其原始字节表示形式。
func TestParsePkScript(t *testing.T) {
	...
	tests := []struct {
		...
		// 无效的 P2PKH - 与上面相同，但用 OP_CHECKSIGVERIFY 替换 OP_CHECKSIG。
		{
			name: "invalid P2PKH",
			pkScript: []byte{
				// OP_DUP
		...
		// 无效的 P2SH - 与上面相同，但将 OP_EQUAL 替换为 OP_EQUALVERIFY。
		{
			name: "invalid P2SH",
			pkScript: []byte{
				// OP_HASH160
...
// TestComputePkScript 确保我们可以通过查看尝试使用输入的签名脚本/见证人来正确地重新导出输出的 pkScript。
func TestComputePkScript(t *testing.T) {
	...
	tests := []struct {
		...
		// 无效的 v0 P2WPKH - 与上面相同，但公钥上缺少一个字节。
		{
